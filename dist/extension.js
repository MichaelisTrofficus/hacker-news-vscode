...
async function getLatestNews() {
    try {
        ...
        const stories = await Promise.all(storyIds.map(async (storyId) => {
            const storyResponse = await axios_1.default.get(`https://hacker-news.firebaseio.com/v0/item/${storyId}.json`);
 ...
 */
function createNewEntry(entryObject, index) {
    const { id, url, title, score, by, descendants } = entryObject;
    const rank = index + 1;
    let hostname = '';
    if (url) {
 ...
 */
function getWebviewContent(styleSrc, entriesBlock) {
    return `
...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('hacker-news-vscode.getLatestHackerNews', async () => {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('hacker-news-vscode', 'Hacker News', vscode.ViewColumn.One, {});
